...
void RLTAS_HotkeyTick(){g_TAS.Tick();if(edge(VK_F9))g_TAS.enabled=!g_TAS.enabled;if(!g_TAS.enabled)return;if(edge('R'))g_TAS.SaveNow();if(edge(VK_XBUTTON2))g_TAS.RestoreSaved();if(edge(VK_XBUTTON1))g_TAS.RewindOneSecond();if(edge('6')){g_TAS.gravityLevel=std::max(-1,g_TAS.gravityLevel-1);g_TAS.ApplyPhysics();}if(edge('7')){g_TAS.gravityLevel=std::min(2,g_TAS.gravityLevel+1);g_TAS.ApplyPhysics();}if(edge('F')){g_TAS.bounceLevel=std::min(2,g_TAS.bounceLevel+1);g_TAS.ApplyPhysics();}if(edge('G')){g_TAS.bounceLevel=std::max(-2,g_TAS.bounceLevel-1);g_TAS.ApplyPhysics();}if(edge('8'))g_TAS.ResetPhysics();if(edge('5'))g_TAS.recording=!g_TAS.recording;if(edge('9'))g_TAS.recording=false;}
